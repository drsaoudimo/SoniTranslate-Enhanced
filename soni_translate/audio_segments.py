...
def create_translated_audio(result_diarize, audio_files, final_file, concat=False, avoid_overlap=False):
    ...
    if concat:
        ...
        # Create silent audio with total_duration
        ...
        logger.debug(f"Audio duration: {total_duration // 60} minutes and {int(total_duration % 60)} seconds")
            ...
            try:
                ...
                if avoid_overlap:
                    ...
                    if (last_end_time - 0.500) > start:
                        ...
                        if overlap_time > 2.5:
                            start -= 0.3
                        logger.info(f"Avoid overlap for {audio_file} with {start}")

                    previous_speaker = speaker
                    duration_tts_seconds = len(audio) / 1000.0  # to seconds
                    last_end_time = start + duration_tts_seconds

                start_time = start * 1000  # to milliseconds
                combined_audio.overlay(audio, position=start_time)
            except Exception as error:
                logger.debug(str(error))
                logger.error(f"Error with audio file {audio_file}")

        # Export combined audio as a file
        combined_audio_data = combined_audio.to_audio_segment()
        combined_audio_data.export(final_file, format="wav")  # use wav format for better quality
